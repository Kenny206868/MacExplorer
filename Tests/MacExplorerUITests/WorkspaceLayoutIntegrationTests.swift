...
final class WorkspaceLayoutIntegrationTests: XCTestCase {
    ...
    @MainActor func testActualSwiftUILayoutDoesNotOverlapOrOverflow() async throws {
        ...
        for width in [800.0, 860, 960, 1024, 1200, 1240, 1440, 1600, 1920] {
            for flags in 0..<4 {
                preferences.value.inspector = flags & 1 != 0; preferences.value.previewPane = flags & 2 != 0
                ...
                let required: Set<String> = ["tabs", "address", "sidebar", "files", "status"]
                ...
                for (name, rect) in regions.values {
                    XCTAssertGreaterThan(rect.width, 0, "\(name): \(label)"); XCTAssertGreaterThan(rect.height, 0, "\(name): \(label)")
                    XCTAssertGreaterThanOrEqual(rect.minX, -1, "\(name): \(label)"); XCTAssertGreaterThanOrEqual(rect.minY, -1, "\(name): \(label)")
                    XCTAssertLessThanOrEqual(rect.maxX, width + 1, "\(name): \(label)"); XCTAssertLessThanOrEqual(rect.maxY, 701, "\(name): \(label)")
                }
                ...
                if flags == 3 { XCTAssertEqual(regions.values["auxiliary"] != nil, width < 1240, label) }
