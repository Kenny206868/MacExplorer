...
final class WorkspaceChromeControlsTests: XCTestCase {
    ...
    @MainActor func testNativeHostResizesWithoutReplacingToolbarOrCustomItems() throws {
        ...
        let paletteItem = try XCTUnwrap(chrome.toolbar(toolbar, itemForItemIdentifier: .init("workspace"), willBeInsertedIntoToolbar: false))
        let paletteHost = try XCTUnwrap(paletteItem.view)
        XCTAssertFalse(paletteHost === host)
        for width in [800.0, 1260, 1760, 1024] {
            ...
            let paletteWidth = try XCTUnwrap(paletteHost.constraints.first { $0.firstAttribute == .width && $0.relation == .equal })
            XCTAssertEqual(paletteWidth.constant, constraint.constant, "Customization previews must not steal the installed item's resize state")
