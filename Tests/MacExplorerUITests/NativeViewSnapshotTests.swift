...
/// Production SwiftUI views, not the HTML design. The fixture uses the exact
/// URLs installed in FileEntry so folder URL hints cannot hide a selected row.
final class NativeViewSnapshotTests: XCTestCase {
    ...
    @MainActor func testNativeViewMatrix() async throws {
        guard let outputPath = ProcessInfo.processInfo.environment["MACEXPLORER_SNAPSHOT_DIR"] else { throw XCTSkip("Set MACEXPLORER_SNAPSHOT_DIR to capture native view fixtures.") }
        ...
        let defaultsKey = "MacExplorer.preferences.v1", previousData = UserDefaults.standard.data(forKey: "MacExplorer.preferences.v1")
        let preferences = PreferenceStore.shared, originalPreferences = PreferenceStore.shared.value, originalJobs = OperationCenter.shared.jobs
        defer {
            preferences.value = originalPreferences
            if let previousData { UserDefaults.standard.set(previousData, forKey: defaultsKey) } else { UserDefaults.standard.removeObject(forKey: defaultsKey) }
            OperationCenter.shared.jobs = originalJobs; try? FileManager.default.removeItem(at: fixtureRoot)
        }
        ...
        let poster = fixture.appendingPathComponent("Color study.png"), image = NSImage(size: NSSize(width: 960, height: 640))
        image.lockFocus()
        NSColor(calibratedRed: 0.12, green: 0.20, blue: 0.30, alpha: 1).setFill(); NSRect(x: 0, y: 0, width: 960, height: 640).fill()
        NSColor(calibratedRed: 0.30, green: 0.75, blue: 0.72, alpha: 1).setFill(); NSBezierPath(roundedRect: NSRect(x: 100, y: 110, width: 530, height: 400), xRadius: 64, yRadius: 64).fill()
        NSColor(calibratedRed: 0.97, green: 0.74, blue: 0.39, alpha: 1).setFill(); NSBezierPath(ovalIn: NSRect(x: 490, y: 170, width: 330, height: 330)).fill(); image.unlockFocus()
        let representation = try XCTUnwrap(image.tiffRepresentation.flatMap(NSBitmapImageRep.init(data:)))
        try XCTUnwrap(representation.representation(using: .png, properties: [:])).write(to: poster); urls.append(poster)
        let entries = try urls.map { try FileEntry(url: $0) }
        var settings = Preferences(); settings.pins = entries.prefix(3).map { Bookmark($0.url) }; settings.recent = []
        settings.tabs = [.folder(fixture)]; settings.restoreTabs = false; preferences.value = settings
        ...
        let tab = workspace.current; tab.stop(); tab.entries = entries; tab.loading = false
        let updater = AppUpdater()
        var captures: [Capture] = []
        func shell() -> AnyView { AnyView(WorkspaceShell(workspace: workspace, tab: tab).environmentObject(workspace).environmentObject(preferences).environmentObject(updater)) }
        for dark in [false, true] {
            let theme = dark ? "dark" : "light"
            preferences.value.theme = theme; preferences.value.inspector = true; preferences.value.previewPane = false
            preferences.value.checkboxes = false; OperationCenter.shared.jobs = []; tab.options.group = .none
            for mode in ViewMode.allCases {
                tab.options.view = mode; tab.selection = [mode == .gallery ? entries[entries.count - 1].url : entries[3].url]
                XCTAssertEqual(workspace.selected.count, 1)
            ...
            tab.options.view = .large; tab.options.group = .kind; preferences.value.checkboxes = true
            tab.selection = Set(entries.prefix(4).map(\.url))
            XCTAssertEqual(workspace.selected.count, 4, "The actual selection includes three folders and one file")
            ...
            tab.options.group = .none; preferences.value.previewPane = true; tab.selection = []
            for width in [800, 1024, 1600] { captures.append(try await capture(shell(), named: "\(theme)-panes-\(width)", size: NSSize(width: width, height: 700), dark: dark, output: output)) }
            preferences.value.previewPane = false; preferences.value.inspector = false; tab.entries = []
            ...
            tab.error = nil; tab.entries = entries; tab.selection = [entries[3].url, entries[4].url]
