...
final class CoreTests: XCTestCase {
    ...
    private func file(_ name: String, text: String = "sample") throws -> URL {
        ...
        try Data(text.utf8).write(to: url, options: .withoutOverwriting); return url
    }
    ...
    func testHistoryTruncatesForwardBranch() {
        var h = NavigationHistory(.home)
        h.navigate(.computer); h.navigate(.network); h.back(); h.navigate(.trash)
    ...
    func testUniqueNamesPreserveExtension() throws {
        let url = try file("Report.pdf")
        XCTAssertEqual(FileNames.unique(url).lastPathComponent, "Report (2).pdf")
    }
    func testMetadataSearch() throws {
        ...
        XCTAssertFalse(SearchExpression("ext:png").matches(entry))
        XCTAssertFalse(SearchExpression("size:<1KB").matches(entry))
    }
    func testNaturalSortAndFolderFirst() throws {
        let a = try FileEntry(url: file("file10.txt")), b = try FileEntry(url: file("file2.txt"))
    ...
    func testSHA256Streaming() async throws {
        let url = try file("checksum.txt", text: "abc")
        let hash = try await FileService().checksum(url)
    ...
    func testCopyKeepBothAndReceipt() async throws {
        ...
        let result = await FileOperationEngine().run(FileJob(.copy, sources: [source], destination: root), control: OperationControl(), progress: { _ in }, resolve: { _ in CollisionAnswer(.keepBoth) })
        XCTAssertTrue(result.errors.isEmpty); XCTAssertEqual(result.outputs.first?.lastPathComponent, "source (2).txt")
        XCTAssertEqual(result.receipt.steps.count, 1)
        XCTAssertTrue(FileNames.exists(source))
    }
    func testMoveAndUndo() async throws {
        let engine = FileOperationEngine(), source = try file("move.txt")
        ...
        XCTAssertTrue(result.errors.isEmpty); XCTAssertFalse(FileNames.exists(source))
        let undone = await engine.undo(result.receipt, control: OperationControl())
        XCTAssertTrue(undone.errors.isEmpty); XCTAssertTrue(FileNames.exists(source))
    }
    func testChangedFileCannotBeUndone() async throws {
        let engine = FileOperationEngine(), source = try file("change.txt")
        let destination = root.appendingPathComponent("destination")
        try FileManager.default.createDirectory(at: destination, withIntermediateDirectories: false)
        let result = await engine.run(FileJob(.move, sources: [source], destination: destination), control: OperationControl(), progress: { _ in }, resolve: { _ in CollisionAnswer(.cancel) })
        let output = try XCTUnwrap(result.outputs.first)
        try Data("newer contents are protected".utf8).write(to: output)
        let undone = await engine.undo(result.receipt, control: OperationControl())
    ...
    func testRenameSwapsWithoutOverwriting() async throws {
        ...
        let result = await FileOperationEngine().rename([(a, "b.txt"), (b, "a.txt")], control: OperationControl())
