...
MANIFESTS = ('captures.json', 'archive-captures.json', 'design-captures.json', 'dual-captures.json', 'settings-captures.json', 'inline-captures.json')
CASES = {'extra-large-icons', 'large-icons', 'medium-icons', 'small-icons', 'list', 'details',
         ...
         'touch-file-actions', 'keyboard-help', 'settings-appearance', 'settings-input', 'settings-integration', 'settings-updates', 'inline-rename'}
...
def validate(root: pathlib.Path) -> dict:
    ...
    report = {'schemaVersion': 5, 'captures': len(captures), 'sha256': checksums,
              'checks': ['coverage', 'PNG dimensions', 'nonblank rendering', 'opaque sRGB compositing',
                         ...
                         'responsive orientation intent', 'touch and keyboard action coverage', 'all settings pages', 'native inline filename editor']}
