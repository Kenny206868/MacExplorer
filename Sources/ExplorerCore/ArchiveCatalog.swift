...
/// Read options are deliberately not Codable: archive passwords never enter
/// preferences, recovery receipts, diagnostics, process arguments, or manifests.
...
public struct ArchiveCatalog: Sendable {
    ...
    /// Direct children, including implicit directories omitted from ZIP headers.
        ...
        for member in members where member.path.hasPrefix(prefix) {
            ...
            if let separator = remainder.firstIndex(of: "/") {
                ...
                if children[path] == nil {
                    children[path] = ArchiveMember(path: path, isDirectory: true, size: 0, encrypted: false, modified: nil, unsupportedReason: nil)
                }
    ...
    public static func read(_ source: URL, options: ArchiveReadOptions = ArchiveReadOptions(),
        ...
        while let entry = try reader.next(control: control) {
            ...
            members.append(ArchiveMember(path: path, isDirectory: type == 0o040000, size: size,
                                         ...
                                         modified: ArchiveReader.modificationDate(entry), unsupportedReason: unsupported))
        }
...
public enum ArchivePath {
    /// Accept conventional ./tar paths but never reinterpret traversal, absolute
    /// names, drive syntax, alternate separators, or embedded special components.
    public static func parse(_ name: String, directory: Bool) throws -> String? {
        guard !name.hasPrefix("/"), !name.contains("\\"), !name.contains(":"), !name.contains("\0"), name.utf8.count <= 8192 else {
            throw ExplorerError.message("Unsafe archive path: \(name)")
        }
...
final class ArchiveReader {
    ...
    init(source: URL, options: ArchiveReadOptions) throws {
        handle = try Self.open(source: source, options: options)
    }
    private static func open(source: URL, options: ArchiveReadOptions) throws -> OpaquePointer {
        ...
        guard archive_read_open_filename(handle, source.path, 65_536) == ARCHIVE_OK else {
            let error = Self.error(handle); archive_read_free(handle); throw error
        }
        return handle
    }
    ...
    func next(control: OperationControl) throws -> OpaquePointer? {
        ...
        guard status == ARCHIVE_OK, let entry else { throw Self.error(handle) }
        return entry
    }
    func name(_ entry: OpaquePointer) throws -> String {
        guard let value = archive_entry_pathname(entry) else { throw ExplorerError.message("Archive entry has no name.") }
    ...
    static func modificationDate(_ entry: OpaquePointer) -> Date? {
        ...
        guard value.isFinite, (-62_135_596_800...253_402_300_799).contains(value) else { return nil }
        return Date(timeIntervalSince1970: value)
    }
    static func error(_ handle: OpaquePointer) -> ExplorerError {
        .message(archive_error_string(handle).map { String(cString: $0) } ?? "Archive operation failed.")
    }
}
