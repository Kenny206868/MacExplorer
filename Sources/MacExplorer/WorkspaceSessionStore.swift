...
/// One recoverable window. Archive passwords never enter this value model.
struct WindowSession: Codable, Hashable, Identifiable, Sendable {
    ...
    init(id: UUID = UUID(), saved: Date = Date(), tabs: [BrowserSession], activeIndex: Int,
         closedTabs: [BrowserSession] = [], frame: CGRect? = nil) {
    ...
    private static func bounded(_ tab: BrowserSession) -> BrowserSession {
        BrowserSession(id: tab.id, history: tab.history, options: tab.options,
            query: String(tab.query.prefix(8192)), allLocations: tab.allLocations,
            selection: Array(tab.selection.prefix(2048)))
    }
    private enum CodingKeys: String, CodingKey { case id, saved, tabs, activeIndex, closedTabs, frame }
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        id = try c.decode(UUID.self, forKey: .id); saved = try c.decode(Date.self, forKey: .saved)
        tabs = try c.decode([BrowserSession].self, forKey: .tabs)
    ...
    func newIdentity() -> WindowSession { WindowSession(tabs: tabs, activeIndex: activeIndex, closedTabs: closedTabs, frame: frame) }
}
...
/// Encoding occurs on MainActor; atomic disk replacement is serialized off it.
/// A flush is reserved for application termination and explicit tests.
...
@MainActor final class WorkspaceSessionCoordinator: ObservableObject {
    ...
    func claimInitialWindow(restore: Bool) -> InitialWindow {
        guard !didClaimInitial else { return .fresh }
        didClaimInitial = true
    ...
    func restoreAdditionalWindows(using open: (WindowSession) -> Void) {
        guard didClaimInitial, !didRequestAdditional else { return }
        didRequestAdditional = true
    ...
    func record(_ workspace: ExplorerWorkspace) {
        guard live[workspace.id] != nil, !quitting else { return }
        persist()
    }
    func close(_ workspace: ExplorerWorkspace) {
        ...
        if !quitting {
            closedWindows.insert(snapshot(workspace), at: 0)
            closedWindows = Array(closedWindows.prefix(12))
            live.removeValue(forKey: workspace.id); order.removeAll { $0 == workspace.id }
            persist()
        }
    }
    func reopen(_ id: UUID? = nil) -> WindowSession? {
        let found: Int?
        if let id { found = closedWindows.firstIndex { $0.id == id } }
        else { found = closedWindows.isEmpty ? nil : 0 }
    ...
    func snapshot(_ workspace: ExplorerWorkspace) -> WindowSession {
        WindowSession(id: workspace.id, tabs: workspace.tabs.map { $0.session() },
            ...
            closedTabs: workspace.closedTabs, frame: workspace.window?.frame)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                ...
                Spacer()
                Button("Clear History") { sessions.clearClosedWindows() }.disabled(sessions.closedWindows.isEmpty)
            }.padding(24)
            Divider()
            if sessions.closedWindows.isEmpty {
                ...
                List(sessions.closedWindows) { session in
                    HStack(spacing: 16) {
                        Image(systemName: "macwindow").font(.title2).foregroundStyle(.tint)
                        VStack(alignment: .leading, spacing: 5) {
                            Text(session.title).fontWeight(.semibold).lineLimit(1)
                            Text("\(session.tabs.count) tabs · " + session.saved.formatted(date: .abbreviated, time: .shortened)).font(.caption).foregroundStyle(.secondary)
                            Text(session.tabs.map { $0.history.current.title }.joined(separator: " · ")).font(.caption).foregroundStyle(.secondary).lineLimit(2)
                        }
                        Spacer()
                        Button("Restore Window") { if let value = sessions.reopen(session.id) { openWindow(id: "restored-session", value: value) } }
                    }.padding(.vertical, 10)
                }
            }
