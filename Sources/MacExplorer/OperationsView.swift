...
struct OperationsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            SheetHeading(title: "File operations", subtitle: "Operations are serialized across every window. Pause and cancellation take effect at safe file boundaries; completed items are retained.")
...
struct OperationCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Image(systemName: job.finished ? (job.errors.isEmpty ? "checkmark.circle.fill" : "exclamationmark.triangle.fill") : "doc.on.doc").foregroundStyle(job.errors.isEmpty ? Color.accentColor : Color.orange)
                Text(job.title).fontWeight(.semibold); Spacer()
                if !job.finished {
                    Button { job.togglePause() } label: { Image(systemName: job.paused ? "play.fill" : "pause.fill") }.help(job.paused ? "Resume" : "Pause at the next file boundary")
                    Button { job.cancel() } label: { Image(systemName: "xmark") }.help("Cancel remaining work")
                }
            }
            if !job.finished { ProgressView(value: Double(job.progress.completed), total: Double(max(1, job.progress.total))) }
            HStack { Text(job.status); Spacer(); if job.progress.total > 1 { Text("\(job.progress.completed) / \(job.progress.total) items").monospacedDigit() } }.font(.caption).foregroundStyle(.secondary)
