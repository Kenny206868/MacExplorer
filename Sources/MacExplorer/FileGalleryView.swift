...
private final class GalleryRaster: @unchecked Sendable { let image: CGImage; init(_ image: CGImage) { self.image = image } }
private actor GalleryRasterCache {
    ...
    func image(for entry: FileEntry) throws -> GalleryRaster {
        ...
        guard entry.isDownloaded, !entry.canBrowse,
              ...
              let image = CGImageSourceCreateThumbnailAtIndex(source, 0, [kCGImageSourceCreateThumbnailFromImageAlways: true, kCGImageSourceCreateThumbnailWithTransform: true, kCGImageSourceShouldCacheImmediately: true, kCGImageSourceThumbnailMaxPixelSize: 2048] as CFDictionary) else {
            throw ExplorerError.message("Use Quick Look to preview this image format.")
        }
        try Task.checkCancellation()
        let raster = GalleryRaster(image); cache.setObject(raster, forKey: key, cost: image.bytesPerRow * image.height); return raster
    }
}
struct FileGalleryView: View {
    ...
    private var active: FileEntry? { files.first { $0.url == tab.focusedURL && tab.selection.contains($0.url) } ?? files.first { tab.selection.contains($0.url) } ?? files.first }
            ...
            if let entry = active {
                HStack(spacing: 8) {
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text(entry.conciseKind + " · " + entry.compactSizeText).font(.caption).foregroundStyle(.secondary).lineLimit(1)
                    }
                    Spacer(minLength: 4)
                ...
                LazyHStack(spacing: 8) {
                    ForEach(files) { entry in
                        VStack(spacing: 8) {
                            ...
                            FileNameLabel(entry: entry, tab: tab, centered: true).font(.system(size: 10))
                        }.frame(width: 112, height: 96).padding(6)
        ...
        VStack(spacing: 0) {
            GeometryReader { proxy in
                if let raster {
                    ...
                    let width = CGFloat(rotated ? raster.image.height : raster.image.width), height = CGFloat(rotated ? raster.image.width : raster.image.height)
                    ...
                    ScrollView([.horizontal, .vertical]) {
                        ...
                        .onEnded { if input.gesturesEnabled { zoom = min(8, max(1, zoom * $0.magnification)) } }).accessibilityLabel("Image preview: " + entry.name)
