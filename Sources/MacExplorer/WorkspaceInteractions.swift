...
@MainActor extension ExplorerWorkspace {
    func select(_ url: URL, extend: Bool, range: Bool) {
        let tab = current, ordered = current.navigableEntries.map(\.url)
        guard ordered.contains(url) else { return }
    ...
    func selectAll() { current.rangeBaseline = nil; current.selection = Set(current.navigableEntries.map(\.url)) }
    func invertSelection() { current.rangeBaseline = nil; current.selection = Set(current.navigableEntries.map(\.url)).subtracting(current.selection) }
    func moveSelection(_ offset: Int, extend: Bool = false, focusOnly: Bool = false) {
        let tab = current, files = current.navigableEntries
        tab.typeAhead.reset(); guard !files.isEmpty else { return }
    ...
    func typeToSelect(_ text: String, time: TimeInterval = ProcessInfo.processInfo.systemUptime) -> Bool {
        let tab = current, files = current.navigableEntries
        let currentIndex = files.firstIndex { $0.url == tab.focusedURL } ?? files.firstIndex { tab.selection.contains($0.url) }
        guard let index = tab.typeAhead.match(text, names: files.map(\.name), focusedIndex: currentIndex, time: time) else { return false }
        select(files[index].url, extend: false, range: false); return true
    }
    ...
    func keyboardMove(_ key: UInt16, shift: Bool, control: Bool) {
        ...
        if key == 115 || key == 119 { selectBoundary(last: key == 119, extend: shift, focusOnly: control && !shift) }
            ...
            if stride > 1 && (key == 125 || key == 126) {
                let files = current.navigableEntries
                let index = files.firstIndex { $0.url == current.focusedURL } ?? files.firstIndex { current.selection.contains($0.url) }
                if let index, let neighbor = GridNavigation.verticalNeighbor(of: index, counts: current.groups.map { $0.1.count }, columns: stride, direction: key == 126 ? -1 : 1) { delta = neighbor - index }
            }
    ...
    func toggleFocusedSelection() {
        guard let url = current.focusedURL ?? current.navigableEntries.first?.url else { return }; select(url, extend: true, range: false)
    }
}
