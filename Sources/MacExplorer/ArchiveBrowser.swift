...
@MainActor final class ArchiveBrowserModel: ObservableObject {
    ...
    var children: [ArchiveMember] { (catalog?.children(of: folder) ?? []).filter { query.isEmpty || $0.name.localizedStandardContains(query) } }
    func scan() {
        ...
        scanTask = Task {
            do {
                ...
                guard !Task.isCancelled else { return }; catalog = value; loading = false
    ...
    func extract(all: Bool, owner: ExplorerWorkspace) {
        ...
        owner.operations.submit(FileJob(.extract, sources: [source], destination: target, archiveOptions: options), owner: owner) { [weak self] result in
            guard let self else { return }; extracting = false
...
struct ArchiveBrowserSheet: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 12) {
                Image(systemName: "doc.zipper").font(.system(size: 30)).foregroundStyle(Color.accentColor)
                ...
                Button("Done") { dismiss() }.buttonStyle(ExplorerButtonStyle()).keyboardShortcut(.cancelAction)
            }.padding(20)
            ExplorerRule()
            HStack(spacing: 10) {
                CommandIcon("Parent folder", "arrow.up", disabled: model.folder.isEmpty) { model.navigate(model.folder.split(separator: "/").dropLast().joined(separator: "/")) }
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 6) {
                        ...
                        ForEach(Array(components.enumerated()), id: \.offset) { index, name in
                            Image(systemName: "chevron.right").font(.caption2).foregroundStyle(ExplorerDesign.muted)
            ...
            }.padding(.horizontal, 16).padding(.vertical, 8).background(ExplorerDesign.chrome)
            ExplorerRule()
            Table(model.children, selection: $model.selection) {
                TableColumn("Name") { member in
                    Label(member.name, systemImage: member.isDirectory ? "folder.fill" : "doc")
                        .foregroundStyle(member.unsupportedReason == nil ? ExplorerDesign.text : Color.orange).help(member.unsupportedReason ?? member.path)
                }.width(min: 240, ideal: 350)
                TableColumn("Size") { member in Text(member.isDirectory ? "—" : ByteCountFormatter.string(fromByteCount: member.size, countStyle: .file)).monospacedDigit() }.width(min: 70, ideal: 100)
                TableColumn("Modified") { member in Text(member.modified.map { $0.formatted(date: .abbreviated, time: .shortened) } ?? "—") }.width(min: 140, ideal: 170)
                TableColumn("Protection") { member in if member.encrypted { Label("Encrypted", systemImage: "lock").font(.caption) } }.width(min: 80, ideal: 100)
            }.alternatingRowBackgrounds(.disabled).scrollContentBackground(.hidden).background(ExplorerDesign.canvas)
                .contextMenu(forSelectionType: String.self) { selection in
                    Button("Extract Selection") { model.selection = selection; model.extract(all: false, owner: workspace) }.disabled(selection.isEmpty || model.extracting)
                } primaryAction: { selection in
                    if selection.count == 1, let path = selection.first, model.children.first(where: { $0.path == path })?.isDirectory == true { model.navigate(path) }
                }
                .overlay {
                    if model.catalog == nil && !model.loading {
                        ContentUnavailableView("Archive contents unavailable", systemImage: "doc.zipper", description: Text("Enter a password when required, then reload the archive."))
                    }
                }
            ExplorerRule()
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack {
                    ...
                    Button("Reload") { model.scan() }.buttonStyle(ExplorerButtonStyle()).disabled(model.loading || model.extracting)
                }
                if let error = model.error { Label(error, systemImage: "exclamationmark.triangle").font(.caption).foregroundStyle(.red).textSelection(.enabled).lineLimit(3) }
                if let output = model.output {
                    HStack {
                        Label("Extraction complete", systemImage: "checkmark.circle.fill").foregroundStyle(.green); Spacer()
                        Button("Open Folder") { workspace.newTab(.folder(output)); dismiss() }.buttonStyle(ExplorerButtonStyle())
                    }.font(.callout)
                }
                HStack {
                    Text(model.selection.isEmpty ? "Passwords are kept only in memory. Format support depends on macOS." : "\(model.selection.count) selected · folder selections include their contents")
                        .font(.caption2).foregroundStyle(ExplorerDesign.muted)
                    Spacer(minLength: 10)
                    if model.extracting {
                        ProgressView().controlSize(.small); Button("Show Progress") { workspace.sheet = .operations }.buttonStyle(ExplorerButtonStyle())
                    } else {
                        Button("Extract Selected") { model.extract(all: false, owner: workspace) }.buttonStyle(ExplorerButtonStyle()).disabled(model.selection.isEmpty || model.loading)
                        Button("Extract All") { model.extract(all: true, owner: workspace) }.buttonStyle(ExplorerButtonStyle(primary: true)).disabled(model.loading)
                    }
                }
            }.padding(20)
        }.frame(width: 840, height: 650).foregroundStyle(ExplorerDesign.text).background(ExplorerDesign.canvas)
