...
struct FileTile: View {
    ...
    var body: some View {
        interactiveSurface.contextMenu { FileContextMenu(workspace: workspace, urls: menuURLs) }
            ...
            .accessibilityAction { workspace.open(entry) }.help(entry.url.path)
    }
    ...
    @ViewBuilder private var tileContent: some View {
        if horizontal {
            ...
            }.padding(.horizontal, 10).frame(height: tab.options.view == .small ? 36 : 80)
        } else {
            VStack(spacing: 10) {
...
struct FileWideRow: View {
    ...
    private var surface: some View {
        HStack(spacing: 12) {
            if preferences.value.checkboxes { Toggle("Select " + entry.name, isOn: Binding(get: { selected }, set: { _ in workspace.select(entry.url, extend: true, range: false) })).labelsHidden().toggleStyle(.checkbox) }
            FileArtwork(entry: entry, size: detailed ? 42 : 20)
            VStack(alignment: .leading, spacing: 5) {
                Text(displayName(entry, extensions: preferences.value.showExtensions)).font(.system(size: 12)).lineLimit(1).truncationMode(.middle).foregroundStyle(ExplorerDesign.text)
        ...
        }.padding(.horizontal, 12).frame(height: detailed ? 70 : preferences.value.compact ? 28 : 36)
